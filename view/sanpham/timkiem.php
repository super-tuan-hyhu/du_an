...
$sql = "SELECT * FROM san_pham WHERE ten_sp LIKE '%$key%' $fill";
$sps = pdo_query($sql);

$bd = ($page-1)*3;
$kt = $bd + 3;

$sql = "SELECT * FROM san_pham WHERE ten_sp LIKE '%$key%' $fill LIMIT $bd, $kt";
$sp = pdo_query($sql);
?>
<main style="transform: none; margin-bottom: 388.2px;">
    ...
    <!-- /filter_type -->
	            ...
	            <div class="col-lg-9">
	                <h2>Mẫu Áo Mà Bạn Tìm Kiếm "<?php if(isset($_SESSION['key'])) echo $_SESSION['key'] ?>"</h2>
	                <!-- /top_banner -->
	               ...
	               <div class="row small-gutters">
								...
								<div class="modal product-modal" tabindex="-1" id="m<?php echo $key ?>">
									...
									<div class="modal-body">
										<form class="body" method="post">
											...
											<a href="<?php $row['ma_sp'] ?>"><button type="button" class="btn btn-secondary" name="addTocart" data-bs-dismiss="modal">Close</button></a>
					...
					<div class="paginationn">
						<?php for($i=1;$i<=ceil(count($sps)/3);$i++): ?>
							<a href="index.php?act=timkiem&page=<?php echo $i?>" class="page-linkk"><?php echo $i?></a>
						<?php endfor ?>
					</div>
	            </div>
	        </div>
